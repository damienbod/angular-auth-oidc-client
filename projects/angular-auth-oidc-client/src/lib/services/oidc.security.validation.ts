...
// Core 1.0 [OpenID.Core] specification.
//
...
// access_token C3: The value of at_hash in the ID Token MUST match the value produced in the previous step if at_hash is present in the
// ID Token.
...
export class OidcSecurityValidation {
    ...
    // iss
    // REQUIRED. Issuer Identifier for the Issuer of the response.The iss value is a case-sensitive URL using the https scheme that
    ...
    // Its value is a JSON [RFC7159] number representing the number of seconds from 1970- 01 - 01T00: 00:00Z as measured in
    ...
    // id_token C8: The iat Claim can be used to reject tokens that were issued too far away from the current time,
    // limiting the amount of time that nonces need to be stored to prevent attacks.The acceptable range is Client specific.
    validate_id_token_iat_max_offset(dataIdToken: any, maxOffsetAllowedinSeconds: number): boolean {
        if (!dataIdToken.hasOwnProperty('iat')) {
            return false;
        }
