...
import { AutoRedirectRoutesGuard } from './auto-redirect-routes.guard';

describe(`AutoRedirectRoutesGuard`, () => {
  let autoRedirectRoutesGuard: AutoRedirectRoutesGuard;
  ...
  beforeEach(() => {
    ...
    autoRedirectRoutesGuard = TestBed.inject(AutoRedirectRoutesGuard);
  ...
  it('should create', () => {
    expect(autoRedirectRoutesGuard).toBeTruthy();
  });
  ...
  describe('canActivate', () => {
    it(
      ...
      waitForAsync(() => {
        ...
        autoRedirectRoutesGuard.canActivate(null, { url: 'some-url1' } as RouterStateSnapshot);
  ...
  describe('canLoad', () => {
    it(
      'should save current route (empty)',
      waitForAsync(() => {
        spyOn(authStateService, 'areAuthStorageTokensValid').and.returnValue(false);
        const checkSavedRedirectRouteAndNavigateSpy = spyOn(autoLoginService, 'checkSavedRedirectRouteAndNavigate');
        const saveRedirectRouteSpy = spyOn(autoLoginService, 'saveRedirectRoute');

        autoRedirectRoutesGuard.canLoad(null, []);
    ...
    it(
      'should save current route (with Segments)',
      waitForAsync(() => {
        spyOn(authStateService, 'areAuthStorageTokensValid').and.returnValue(false);
        const checkSavedRedirectRouteAndNavigateSpy = spyOn(autoLoginService, 'checkSavedRedirectRouteAndNavigate');
        const saveRedirectRouteSpy = spyOn(autoLoginService, 'saveRedirectRoute');
