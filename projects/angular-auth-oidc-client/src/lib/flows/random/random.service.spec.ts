...
describe('RandomService Tests', () => {
  ...
  it('should be not equal', () => {
    const r1 = randomService.createRandom(45, { configId: 'configId' });
    const r2 = randomService.createRandom(45, { configId: 'configId' });
    expect(r1).not.toEqual(r2);
  });

  it('correct length with high number', () => {
    const r1 = randomService.createRandom(79, { configId: 'configId' });
    const result = r1.length;
    expect(result).toBe(79);
  });

  it('correct length with small number', () => {
    const r1 = randomService.createRandom(7, { configId: 'configId' });
    const result = r1.length;
    expect(result).toBe(7);
  });

  it('correct length with 0', () => {
    const r1 = randomService.createRandom(0, { configId: 'configId' });
    const result = r1.length;
  ...
  for (let index = 1; index < 7; index++) {
    it('Giving back 10 or more characters when called with numbers less than 7', () => {
      ...
      const r1 = randomService.createRandom(requiredLengthSmallerThenSeven, { configId: 'configId' });
