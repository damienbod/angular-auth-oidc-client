...
describe('UrlService Tests', () => {
  ...
  describe('isCallbackFromSts', () => {
    ...
    const hashTests = [
      {
        url: 'https://any.url/?state=somevalue#code=anyvalue',
        redirectUrl: 'https://any.url/?state=somevalue',
        expected: true,
      },
      {
        url: 'https://any.url/?state=somevalue',
        redirectUrl: 'https://any.url/?state=somevalue',
        expected: true,
      },
    ];

    for (const { url, redirectUrl, expected } of hashTests) {
      it(`should return ${expected} for url ${url}`, () => {
        const result = service.isCallbackFromSts(url, {
          redirectUrl,
          checkRedirectUrlWhenCheckingIfIsCallback: true,
        });
