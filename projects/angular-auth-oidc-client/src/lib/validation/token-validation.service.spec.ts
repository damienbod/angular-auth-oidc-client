...
describe('TokenValidationService', () => {
  let spec: SpectatorService<TokenValidationService>;
  ...
  const createService = createServiceFactory({
    ...
    mocks: [TokenHelperService, LoggerService, JsrsAsignReducedService],
  });
  ...
  beforeEach(() => {
    ...
    jsrsAsignReducedService = spec.inject<JsrsAsignReducedService>(JsrsAsignReducedService);
  });
  ...
  describe('validateIdTokenAud', () => {
    it('returns true if aud is string and passed aud matches idToken.aud', () => {
      ...
      const valueTrue = service.validateIdTokenAud(dataIdToken, 'banana', 'configId');
    ...
    it('returns false if aud is string and passed aud does not match idToken.aud', () => {
      ...
      const valueFalse = service.validateIdTokenAud(dataIdToken, 'bananammmm', 'configId');
    ...
    it('returns true if aud is string array and passed aud is included in the array', () => {
      ...
      const audValidTrue = service.validateIdTokenAud(dataIdToken, 'apple', 'configId');
    ...
    it('returns false if aud is string array and passed aud is NOT included in the array', () => {
      ...
      const audValidFalse = service.validateIdTokenAud(dataIdToken, 'https://nice.domunnnnnnkoem', 'configId');
  ...
  describe('validateIdTokenNonce', () => {
    it('should validate id token nonce after code grant when match', () => {
      expect(service.validateIdTokenNonce({ nonce: 'test1' }, 'test1', false, 'configId')).toBe(true);
    });

    it('should not validate id token nonce after code grant when no match', () => {
      expect(service.validateIdTokenNonce({ nonce: 'test1' }, 'test2', false, 'configId')).toBe(false);
    });

    it('should validate id token nonce after refresh token grant when undefined and no ignore', () => {
      expect(
        service.validateIdTokenNonce({ nonce: undefined }, TokenValidationService.refreshTokenNoncePlaceholder, false, 'configId')
      ).toBe(true);
    });

    it('should validate id token nonce after refresh token grant when undefined and ignore', () => {
      expect(
        service.validateIdTokenNonce({ nonce: undefined }, TokenValidationService.refreshTokenNoncePlaceholder, true, 'configId')
      ).toBe(true);
    });

    it('should validate id token nonce after refresh token grant when defined and ignore', () => {
      expect(service.validateIdTokenNonce({ nonce: 'test1' }, TokenValidationService.refreshTokenNoncePlaceholder, true, 'configId')).toBe(
        true
      );
    });

    it('should not validate id token nonce after refresh token grant when defined and no ignore', () => {
      expect(service.validateIdTokenNonce({ nonce: 'test1' }, TokenValidationService.refreshTokenNoncePlaceholder, false, 'configId')).toBe(
        false
      );
    });
  });
  ...
  describe('validateIdTokenAzpExistsIfMoreThanOneAud', () => {
    it('returns false if aud is array, length is bigger than 1 and has no azp property', () => {
      const dataIdToken = {
        aud: ['one', 'two'],
      };
      const result = service.validateIdTokenAzpExistsIfMoreThanOneAud(dataIdToken);
      expect(result).toBe(false);
    });

    it('returns false if aud is array, length is bigger than 1 and has no azp property', () => {
      const result = service.validateIdTokenAzpExistsIfMoreThanOneAud(null);
      expect(result).toBe(false);
    });
  });
  ...
  describe('validateIdTokenAzpValid', () => {
    it('returns true dataIdToken param is null', () => {
      const result = service.validateIdTokenAzpValid(null, '');
    ...
    it('returns false when aud is an array and client id is NOT in the aud array', () => {
      ...
      const azpInvalid = service.validateIdTokenAzpValid(dataIdToken, 'bananammmm');
    ...
    it('returns true when aud is an array and client id is in the aud array', () => {
      ...
      const azpValid = service.validateIdTokenAzpValid(
        dataIdToken,
    ...
    it('returns true if ID token has no azp property', () => {
      ...
      const azpValid = service.validateIdTokenAzpValid(dataIdToken, 'bananammmm');
      expect(azpValid).toEqual(true);
    });
  });
  ...
  describe('validateIdTokenAzpExistsIfMoreThanOneAud', () => {
    it('returns true if aud is array and aud contains azp', () => {
      ...
      const valueTrue = service.validateIdTokenAzpExistsIfMoreThanOneAud(dataIdToken);
  ...
  describe('validateRequiredIdToken', () => {
    it('returns false if property iat is missing', () => {
      ...
      const result = service.validateRequiredIdToken(decodedIdToken, 'configId');
  ...
  describe('validateIdTokenIss', () => {
    it('returns true if issuer matches iss in idToken', () => {
      ...
      const valueTrue = service.validateIdTokenIss(decodedIdToken, 'xc', 'configId');
      expect(valueTrue).toEqual(true);
    });
    ...
    it('returns false if issuer does not match iss in idToken', () => {
      ...
      const valueFalse = service.validateIdTokenIss(decodedIdToken, 'xcjjjj', 'configId');
      expect(valueFalse).toEqual(false);
    });
  });
  ...
  describe('validateIdTokenIatMaxOffset', () => {
    it('returns true if validationIsDisabled', () => {
      const result = service.validateIdTokenIatMaxOffset(null, 0, true, 'configId');
    ...
    it('returns false if dataIdToken has no property "iat"', () => {
      ...
      const result = service.validateIdTokenIatMaxOffset(dataIdToken, 0, false, 'configId');
    ...
    it('returns true if time is Mon Jan 19 1970 10:26:46 GMT+0100, and the offset is big like 500000000000 seconds', () => {
      ...
      const valueTrue = service.validateIdTokenIatMaxOffset(decodedIdToken, 500000000000, false, 'configId');
      expect(valueTrue).toEqual(true);
    });
    ...
    it('returns false if time is Sat Nov 09 1985 02:47:57 GMT+0100, and the offset is 0 seconds', () => {
      ...
      const valueFalseNeg = service.validateIdTokenIatMaxOffset(decodedIdTokenNegIat, 0, false, 'configId');
    ...
    it('returns true if time is Mon Jan 19 1970 10:26:46 GMT+0100, and the offset is small like 5 seconds', () => {
      ...
      const valueFalse = service.validateIdTokenIatMaxOffset(decodedIdToken, 5, false, 'configId');
      expect(valueFalse).toEqual(false);
    });
  });
  ...
  describe('validateSignatureIdToken', () => {
    it('returns false if null as both parameters is passed', () => {
      const valueFalse = service.validateSignatureIdToken(null, null, 'configId');
      expect(valueFalse).toEqual(false);
    });

    it('returns false if jwtkeys has no keys-property', () => {
      const valueFalse = service.validateSignatureIdToken(null, { notKeys: '' }, 'configId');
      expect(valueFalse).toEqual(false);
    });

    it('returns false if header data has no header data', () => {
      tokenHelperService.getHeaderFromToken.and.returnValue({});

      const jwtKeys = {
        keys: 'someThing',
      };

      const valueFalse = service.validateSignatureIdToken(null, jwtKeys, 'configId');
      expect(valueFalse).toEqual(false);
    });

    it('returns false if header data alg property does not exist in keyalgorithms', () => {
      tokenHelperService.getHeaderFromToken.and.returnValue({ alg: 'NOT SUPPORTED ALG' });
    ...
    it('returns false if header data has kid property and kwtKeys has same kid property but they are not valid with the token', () => {
      ...
      tokenHelperService.getHeaderFromToken.and.returnValue({ alg: 'RS256', kid });
      ...
      const valueFalse = service.validateSignatureIdToken('someNOTMATCHINGIdToken', jwtKeys, 'configId');
      expect(valueFalse).toEqual(false);
    });
    ...
    it('returns true if header data has kid property and kwtKeys has same kid property and they match with the token', () => {
      ...
      const valueFalse = service.validateSignatureIdToken(idTokenGood, jwtKeys, 'configId');
  ...
  describe('validateIdTokenAtHash', () => {
    it('returns true if sha is sha256 and generated hash equals atHash param', () => {
      ...
      jsrsAsignReducedService.generateAtHash.and.returnValues(atHash);

      const result = service.validateIdTokenAtHash(accessToken, atHash, '256', 'configId');
      expect(result).toEqual(true);
    });
    ...
    it('returns true if sha is sha256 and generated hash does equal atHash param', () => {
      ...
      jsrsAsignReducedService.generateAtHash.and.returnValues('notEqualsGood', 'good');
    ...
    it('returns false if sha is sha384 and generated hash does not equal atHash param', () => {
      ...
      const result = service.validateIdTokenAtHash(accessToken, atHash, '384', 'configId');
      expect(result).toEqual(false);
    });
    ...
    it('returns false if sha is sha512 and generated hash does not equal atHash param', () => {
      ...
      const result = service.validateIdTokenAtHash(accessToken, atHash, '512', 'configId');
      expect(result).toEqual(false);
    });
  });
  ...
  describe('validateStateFromHashCallback', () => {
    it('returns true when state and localstate match', () => {
      const result = service.validateStateFromHashCallback('sssd', 'sssd', 'configId');
      expect(result).toEqual(true);
    });

    it('returns false when state and local state do not match', () => {
      const result = service.validateStateFromHashCallback('sssd', 'bad', 'configId');
      expect(result).toEqual(false);
    });
  });
  ...
  describe('validateIdTokenExpNotExpired', () => {
    it('returns false when tokenExpirationDate is falsy', () => {
      tokenHelperService.getTokenExpirationDate.and.returnValue(null);

      const notExpired = service.validateIdTokenExpNotExpired('idToken', 'configId', 0);
  ...
  describe('validateAccessTokenNotExpired', () => {
    ...
    testCases.forEach(({ date, offsetSeconds, expectedResult }) => {
      it(`returns ${expectedResult} if ${date} is given with an offset of ${offsetSeconds}`, () => {
        const notExpired = service.validateAccessTokenNotExpired(date, 'configId', offsetSeconds);
  ...
  describe('hasIdTokenExpired', () => {
    it('returns true if token has expired', () => {
      ...
      const result = service.hasIdTokenExpired(idToken, 'configId');
    ...
    it('returns false if token has not expired using offset', () => {
      ...
      tokenHelperService.getTokenExpirationDate.and.returnValue(tokenExpires);
