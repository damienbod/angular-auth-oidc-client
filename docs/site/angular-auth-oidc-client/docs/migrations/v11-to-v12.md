...
The getter for the active configuration was removed as well as the object PublicConfiguration. Since the `AuthWellKnownEndpoints` parameter is now part of the config, the property was replaced with methods to get the configuration directly. Please read below on how to get the currently used configuration.
